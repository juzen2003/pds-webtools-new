...
class COVIMS_0xxx(pdsfile.PdsFile):
    ...
    def OPUS_ID_TO_PRIMARY_LOGICAL_PATH(opus_id):
        ...
        if opus_id in COVIMS_0xxx.LOWER_VERSION_PRIORITIZED:
            return pdsfile.PdsFile.from_logical_path(COVIMS_0xxx.LOWER_VERSION_PRIORITIZED[opus_id])

        # Search using patterns
        paths = opus_id_to_primary_logical_path.all(opus_id)
        patterns = [pdsfile.abspath_for_logical_path(p) for p in paths]
        matches = []
        for pattern in patterns:
            abspaths = pdsfile.PdsFile.glob_glob(pattern, force_case_sensitive=True)
            matches += abspaths

        if len(matches) == 1:
            return pdsfile.PdsFile.from_abspath(matches[0])
        ...
        version_tuples.sort()
        return pdsfile.PdsFile.from_abspath(version_tuples[-1][1])
